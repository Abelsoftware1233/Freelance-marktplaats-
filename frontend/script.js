...
async function renderJobDetail() {
    try {
        ...
        document.getElementById('jobDetailContent').innerHTML = emptyHtml(currentLang, 'Fout bij laden van opdracht.');
    }
}
...
async function handlePostJob() {
    ...
    try {
        ...
        ['jobTitle', 'jobDescription', 'jobBudget', 'jobLocation', 'jobSkills', 'jobDeadline'].forEach(id => document.getElementById(id).value = '');
...
async function applyFreelancerFilters() {
    ...
    try {
        ...
        document.getElementById('freelancersGrid').innerHTML = freelancers.map(f => freelancerCardHtml(f)).join('') || emptyHtml(currentLang, currentLang === 'nl' ? 'Geen freelancers gevonden.' : 'No freelancers found.');
    } catch (error) {
...
async function renderFreelancerDetail() {
    try {
        ...
        if (!u) { container.innerHTML = emptyHtml(currentLang, 'Niet gevonden.'); return; }
        ...
        document.getElementById('freelancerDetailContent').innerHTML = emptyHtml(currentLang, 'Fout bij laden.');
    }
}
...
function renderMyJobsTable(jobs) {
    ...
    tbody.innerHTML = userJobs.map(j => {
        return `<tr>
            ...
            <td>${j.proposals ? j.proposals.length : 0}</td>
...
function renderProposalsTable(proposals) {
    ...
    tbody.innerHTML = userProposals.map(p => {
        return `<tr>
            <td>${p.job ? escapeHtml(p.job.title) : '?'}</td>
            <td>${euro(p.amount)}</td>
...
function renderProfileTab() {
    const u = currentUser;
    const cat = CATEGORIES.find(c => c.id === u.category);
    document.getElementById('profileGridContent').innerHTML = `
        <div class="profile-card">
            <div class="profile-big-avatar">${initials(u.firstName, u.lastName)}</div>
